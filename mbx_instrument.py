...
class Instrument(object):
    ...
    def query_int(self, cmd):                                                   # visa query for int with basic error handling
        ...
        x = str(self.query(cmd))
        try:
            y = int(float(x))                                                   # convert to float before int
        except:
            y = float("NaN")
...
class VNA_Generic(Instrument):
    ...
    def get_freq_list(self):                                                    # query VNA sweep freq list
        ...
        freqList = [28.e9, 29.e9, 30.e9, 31.e9]
        return freqList
    ...
    def get_sparam(self):
        ...
        sComplex = [1.0+1j*1.0, 0.5-1j*2.0, 0.2+1j*0.3, 1.2]
        return sComplex
...
class VNA_Anrtisu_37xxx(VNA_Generic):
    """ Anritsu 37xxx Lightning VNA driver - Tested with Anritsu 37269D VNA """
    ...
    def init_meas(self):                                                        # initialize VNA
        ...
        # save cal state file to REGISTER 1 or change this setting
        ...
        self.port.set_timeout(TIMEOUT_MEMLOAD)                                  # set timeout to TIMEOUT_MEMLOAD
        print("loading cal file...")
        self.query("RC1;CTN;*OPC?")                                             # load state from register 1 and wait
        self.port.set_timeout(timeout_orig)                                     # restore timeout value
        print("setting to logmag...")
        self.write("MAG")                                                       # set display to logmag
        print("continuous trigger on...")
        self.write("CTN")                                                       # set continuous sweep
        return
    ...
    def get_freq_list(self):                                                    # query VNA sweep freq list
        """ query VNA sweep freq list """
        startFreq = self.query_float("SRT?")
        stopFreq = self.query_float("STP?")
        numPts = self.query_int("ONP")
        freqList = list(np.linspace(startFreq, stopFreq, numPts))
        return freqList
    ...
    def single_trigger(self):                                                   # trigger and hold trace
        ...
        timeout_orig = self.port.get_timeout()
        self.port.set_timeout(TIMEOUT_VNASWEEP)                                 # set timeout to TIMEOUT_VNASWEEP
        self.query("HLD;TRS;WFS;*OPC?")                                         # trigger a single sweep and wait until done
        self.port.set_timeout(timeout_orig)                                     # restore timeout value
        return

    def cont_trigger(self):                                                     # enable continuous trigger
        """ enable continuous trigger """
        self.write("CTN")                                                       # put instrument in continuous sweep mode
        return

    def get_sparam(self):
        """ return S-parameters as list of complex numbers """
        self.write("FMA")                                                       # change to ASCII
        sData = self.query("OCD")                                               # get the S-param data in real, imag format
        sStr = str(sData)                                                       # convert from unicode to string
        sStr = str.strip(sStr)                                                  # remove trailing whitespace (cr/lf)
        sStr = re.sub('^#[0-9]+[\\s]*', '', sStr)                               # strip header of #XXXX at the start of the data
        x = re.split('[\\s,]+', sStr)                                           # split comma and whitespace delimited string to individual strings
...
class VNA_Keysight_FieldFox(VNA_Generic):
    """ Keysight/Agilent FieldFox VNA driver - Tested with Agilent N9952 """
    ...
    def init_meas(self):                                                        # initialize VNA
        ...
        self.port.set_timeout(TIMEOUT_MEMLOAD)                                  # set timeout to TIMEOUT_MEMLOAD
        print("loading cal file...")
        self.write("MMEM:LOAD:STAT '%s'" % calfile)                             # load state and cal file
        self.query("*OPC?")                                                     # wait until done
        self.port.set_timeout(timeout_orig)                                     # restore timeout value
        self.write("CALC:PAR1:SEL")                                             # select the first (only) trace
        print("setting to logmag...")
...
class VNA_HP_PNA(VNA_Generic):
    """ HP VNA driver - Tested with HP 8722D PNA """
    ...
    def init_meas(self):                                                        # initialize VNA
        ...
        self.port.set_timeout(TIMEOUT_MEMLOAD)                                  # set timeout to TIMEOUT_MEMLOAD
        print("loading cal file...")
        self.query("OPC? RECA1")                                                # load state from register 1 and wait
        self.port.set_timeout(timeout_orig)                                     # restore timeout value
        print("setting to logmag...")
        self.write("LOGM")                                                      # set display to logmag
        print("continuous trigger on...")
        self.write("CONT")                                                      # set continuous sweep
        return
    ...
    def get_freq_list(self):                                                    # query VNA sweep freq list
        """ query VNA sweep freq list """
        startFreq = self.query_float("STAR?")
        stopFreq = self.query_float("STOP?")
        numPts = self.query_int("POIN?")
        freqList = list(np.linspace(startFreq, stopFreq, numPts))
        return freqList
    ...
    def single_trigger(self):                                                   # trigger and hold trace
        ...
        timeout_orig = self.port.get_timeout()
        self.port.set_timeout(TIMEOUT_VNASWEEP)                                 # set timeout to TIMEOUT_VNASWEEP
        self.query("OPC? SING")                                                 # trigger a single sweep and wait until done
        self.port.set_timeout(timeout_orig)                                     # restore timeout value
        return

    def cont_trigger(self):                                                     # enable continuous trigger
        """ enable continuous trigger """
        self.write("CONT")                                                      # put instrument in continuous sweep mode
        return

    def get_sparam(self):
        """ return S-parameters as list of complex numbers """
        self.write("FORM4")                                                     # change to ASCII
        sData = self.query("OUTPDATA")                                          # get the S-param data in real, imag format
        sStr = str(sData)                                                       # convert from unicode to string
        sStr = str.strip(sStr)                                                  # remove trailing whitespace (cr/lf)
        x = re.split('[\\s,]+', sStr)                                           # split comma and whitespace delimited string to individual strings
        sComplex = []
...
class VNA_Ceyear(VNA_Generic):
    """ Ceyear VNA driver - Tested with Ceyear 3672B """
    ...
    def init_meas(self):                                                        # initialize VNA
        ...
        # save cal state file to C:\MILLIBOX\MBX.CST or change this file name
        # this cal file resides on the VNA hard drive
        calfile = "C:\\MILLIBOX\\MBX.CST"
...
def inst_setup_single(mode, addr):
    ...
    if mode.find("NONE") > -1:
        ...
        inst.inst_type = mode                                                   # override inst_type to mode if not NONE mode
...
driver_list = [
    ...
    ['VNA*',    'KEYSIGHT*',    'P50*',     VNA_Keysight_PNA()],            # Keysight Streamline USB Network Analyzer
    ['VNA*',    'AGILENT*',     'N524*',    VNA_Keysight_PNA()],            # Agilent PNA-X
    ['VNA*',    'AGILENT*',     'E83*',     VNA_Keysight_PNA()],            # Agilent PNA
    ['VNA*',    'KEYSIGHT*',    'N99*',     VNA_Keysight_FieldFox()],       # Keysight FieldFox
    ['VNA*',    'AGILENT*',     'N99*',     VNA_Keysight_FieldFox()],       # Keysight FieldFox
    ['VNA*',    '*',            '872*',     VNA_HP_PNA()],                  # HP PNA (8722D ...)
    ...
    ['VNA*',    'ANRITSU*',     '37*',      VNA_Anrtisu_37xxx()],           # Anritsu Lightning VNA
    ...
    ['VNA*',    'CEYEAR*',      '36*',      VNA_Ceyear()],                  # Ceyear VNA
